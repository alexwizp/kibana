...
import type { IndexPatternsFetcher } from '../../../../../data/server';
...
export abstract class AbstractSearchStrategy {
  ...
  async getFieldsForWildcard<TPayload = unknown>(
    ...
    const kibanaIndexPattern = await indexPatternsService.findByTitle(indexPattern, true);

    if (kibanaIndexPattern) {
      return await kibanaIndexPattern.fields.getAll();
    }
